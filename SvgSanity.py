...
def get_w3_info():
    ...
    for row in rows:
        ...
        content.append(dict(zip(headers, values)))
    return content

def GenerateSvgElementList():
    knownElements = []
    content = get_w3_info()
...
#         return a
#     for line in content:
#         attrs = line['Attributes'].replace(' ', '')
#         attrs = attrs.split(';;')
#         attrs = filter(None,[trymm(at) for at in attrs])
#         print line['Element'].ljust(19) + ' | '.join(attrs)
#         """ It could return also allowed attributes for each element,
#         but I haven't found such a reliable reference and I doubt if it's worth
#         of my effort. However uncomment what's  above to see how it works. """
    ...
    for line in content:
        ...
        en = el.find('>')
        if st >= 0 and en > 0:
            knownElements.append(el[st + 1:en])
    print knownElements
    return knownElements


""" SVG ELEMENTS INFO FROM: www.w3schools.com/svg/svg_reference.asp"""
w3info = """\
